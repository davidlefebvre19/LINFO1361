...
class SoftFlow(Problem):
        ...
        for w, letter in enumerate(state.source_coords):
            if letter != 'z':
                for z in range(len(moves)):
                    ...
                    if state.grid[new_coord[0]][new_coord[1]].isspace() or new_coord == self.dest_coords[w]:
                        # print((w, (moves[z])))
                        yield (w, (moves[z]))
    ...
    def result(self, state, action):
        ...
        if new_coords == self.dest_coords[action[0]]:
    ...
    def goal_test(self, state):
        return all(source_coord == 'z' for source_coord in state.source_coords)
